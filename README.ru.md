...
1. Скачайте **`patcher_ru.exe`** из этого репозитория.
2. Закиньте его в корневую папку игры (туда, где лежит `My Dystopian Robot Girlfriend.exe`).
3. Убедитесь, что игра в этот момент полностью закрыта.
4. Запустите патчер. Он применит изменения за долю секунды и сам закроется в случае успеха.
5. Включайте игру и заходите в меню читов — админка полностью разблокирована!
...
Откройте PowerShell в папке с файлом `patcher_ru.cs` и выполните:

```powershell
& "$env:SystemRoot\Microsoft.NET\Framework64\v4.0.30319\csc.exe" /target:exe /out:patcher_ru.exe patcher_ru.cs
